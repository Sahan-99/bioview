...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($image_name)) {
        ...
        if (empty($error)) {
            // Insert into scanned_images table with status set to 1
            $status = 1;
            $stmt = $conn->prepare("INSERT INTO scanned_images (image_name, description, model_id, file_path, status) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("ssisi", $image_name, $description, $model_id, $image_path, $status);
