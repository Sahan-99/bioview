...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($model_name)) {
        ...
        if (empty($error)) {
            // Insert into 3d_models table
            $stmt = $conn->prepare("INSERT INTO 3d_models (model_name, description, file_path) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $model_name, $description, $model_path);
            ...
            if ($stmt->execute()) {
                ...
                $error = "Failed to add 3D model to database.";
            }
            $stmt->close();
        }
    }
